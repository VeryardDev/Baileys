...
**Do not file security issues as public GitHub issues, Discord posts, or PRs.** A public report is itself an exploit broadcast — many downstream projects pull Baileys directly from `master` or from `github:Veryard/Baileys`, so a vulnerable version is in the wild within minutes.

Use either of these private channels:

1. **Preferred:** [GitHub Security Advisories](https://github.com/Veryard/Baileys/security/advisories/new) on this repository. This gives us a private workspace to coordinate the fix, draft a CVE, and notify downstream projects.
...
- **Pin a release.** Pulling `github:Veryard/Baileys` always grabs `master`. Use a tagged release in production.
